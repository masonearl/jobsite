...
Run `npm test` for 124 simulation, save, globe and construction-activity checks. Open `/__activity-check` on the QA server for 18 rendered-scene checks covering arrivals, worker capacity, excavation, haul cycles, trench/backfill, concrete, lifting, pause, reconstruction and all three campus types. With the QA server running, open `/__campus-check` for 26 browser checks: a full build, inspection holds, pause, resource additions, map/save return, destination isolation, viewport filling, wheel/keyboard/touch navigation, operations controls and graphics fallback. Add `?mobile` for three additional portrait/landscape layout checks. Touch checks use synthetic pointer input and should be supplemented with real-device testing. `/__campus-check?preview&world&mobile` previews the portrait world view. `/__campus-check?preview&site=starbase&seed=complete` previews completed civil works; use `site=terafab` for the fab. Fixtures use memory storage and an accelerated local-only clock.
