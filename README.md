...
Each scenario has 44 dependency-linked work packages across two structures and their support infrastructure. Dispatch all crews once, order long-lead packages, and run the project. Survey, earthwork, utilities, concrete, steel, electrical, mechanical, fit-out and commissioning teams operate autonomously. Cranes, access equipment, concrete pumps and other spreads are shared. Waiting reasons expose the bottleneck. Prioritize work, add capacity, expedite deliveries, or demobilize finished trades to reduce payroll. Formation, utility acceptance, concrete-strength release, pre-energization and handover require explicit inspection release.

The site camera fills the window automatically. **Operations** opens a scrolling drawer for work packages, labor, equipment, procurement and project records; close it to return to the unobstructed site. The superintendent’s desk provides the next dispatch or inspection action. **Follow work** is the initial camera: it tracks the active crew and follows a loaded truck to its fill. **Next crew** switches to another active front; **Parking** frames the 32-bay crew lot, pedestrian walkway and check-in compound. Dragging releases automatic camera following. Inside work automatically hides the enclosure in Follow work; Roof off is also available.
...
Construction visuals are driven by saved package progress; work amounts still come from the project scheduler. The visual cycles are illustrative, not bucket-level production or rigid-body physics. Reload reconstructs terrain and completed elements; walking routes are rebuilt. Pausing stops the activity clock and movement. The 3D campus gains foundations, structural bays, enclosure, plant equipment and fit-out as work advances. Use **Roof off** to inspect internal services and racks. Terafab replaces data-hall fit-out with process utilities, cleanrooms and tool installation. Starbase has a pad, tower and integration building; it does not simulate rocket operations. Regional ground and repeating weather presets change outdoor productivity and crane availability.
