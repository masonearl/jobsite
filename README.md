...
The game opens directly into a full-window 3D globe. Drag to rotate, scroll or pinch to zoom, and choose a marker or project from the list. **World** resets the camera; **US projects** frames all seven destinations. Keyboard users can focus the globe, rotate with arrow keys, and zoom with + / -. **Fullscreen** optionally hides browser controls. Read the selected project brief, then mobilize. You can also [open construction campaigns directly](https://openmud.ai/jobsite-projects). Choose from Stratos in Utah, Stargate in Abilene, Saline and Doña Ana County, AWS New Carlisle, Terafab in Grimes County, or a Starbase civil expansion.

Each scenario has 44 dependency-linked work packages across two structures and their support infrastructure. Press **Start construction** once: material orders, crew/equipment mobilization, work assignments, inspection releases and handover run automatically. Survey, earthwork, utilities, concrete, steel, electrical, mechanical, fit-out and commissioning teams follow the construction sequence. The superintendent adds capacity for parallel work fronts, keeps short-term standby, then demobilizes finished trades. Inspections still require their simulated work; curing, deliveries, weather and protected pedestrian crossings still take time. Existing held work or empty rosters recover when a saved project resumes.

The site camera fills the window automatically. **Operations** is an optional live reference for work packages, labor, equipment, deliveries and project records. Reading it does not pause construction, and it has no required management actions. The superintendent’s desk explains current work or waiting conditions. The initial Site overview shows the destination layout. **Follow work** tracks the active crew and follows a loaded truck to its fill. **Next crew** switches to another active front; **Parking** frames the 32-bay crew lot, pedestrian walkway and check-in compound. Dragging releases automatic camera following. Inside work automatically hides the enclosure in Follow work; Roof off is also available.

Excavator booms, sticks and buckets articulate through cut, swing and loading cycles. Haul trucks carry visible loads, tip at the fill, and return. Dozers spread the fill; grading changes the terrain at the corresponding work fronts. The utility spread works in six short reaches: excavation, bedding and installation, an inspection before cover, backfill/compaction, then density records. Shared inspection capacity can hold a reach. System testing is a separate acceptance package. The second utility corridor stays clear of the buildings. Foundation cuts, forms, reinforcing and a pre-pour check precede concrete placement. Pumps and rotating mixers support strip-by-strip slab placement. Curing takes elapsed time without an assigned crew or pump; strength and anchor release is required before steel erection. Power and cooling plant also need released foundations. Cranes lift individual columns, rafters and plant components into their installed positions. Crews walk between work fronts and wait at the site compound when their assignments are blocked. On a fresh project, cars arrive during mobilization and passengers walk from the parking lot through the pedestrian gate. This initial arrival sequence is not a daily commute/traffic model. Worker bodies use instanced geometry; parked or held equipment does not circulate without an assignment.

Construction visuals are driven by saved package progress; work amounts still come from the project scheduler. The visual cycles are illustrative, not bucket-level production or rigid-body physics. Reload reconstructs terrain and completed elements; walking routes are rebuilt. Pausing stops the activity clock and movement. The 3D campus gains foundations, structural bays, enclosure, plant equipment and fit-out as work advances. Use **Roof off** to inspect internal services and racks. Terafab replaces data-hall fit-out with process utilities, cleanrooms and tool installation. New Starbase attempts have two launch mounts and towers; it does not simulate rocket operations. Regional ground and repeating weather presets change outdoor productivity and crane availability.

Scenario targets and a completion score summarize schedule and cost performance. The guide and package details link published sources for the sequence. Vendor startup, functional tests and integrated systems tests are separate steps before handover. See [Construction process model](docs/JOBSITE_PROCESS_MODEL.md) for the source-to-rule mapping and assumptions. Map, guide, tab hiding and reload pause the project. P pauses; repeated Space input is not needed. Each destination has a separate device save under `openmud-jobsite-campus-v1:`. Version 2 campus states keep this existing storage key. Valid earlier campus saves migrate with their progress and spending intact; newly separated prerequisites needed by started work receive explicit legacy credit. Before replacing the original, the app saves an exact backup under the key plus `:before-process-update`. A failed backup preserves the original. These saves do not migrate or overwrite utility-line saves. Unsupported or unreadable campus saves are preserved, and another tab's write pauses conflicting saves. Export a JSON project record for comparison. A new attempt explicitly replaces only the selected campus save.

**Public facts vs game assumptions:** every project card links its primary source and gives the source date. These are representative construction phases, not surveyed replicas, complete project budgets, current progress feeds, or endorsed depictions. Abilene and Starbase already exist; their scenarios are fictional additional phases. Stratos is presented as a proposal. Layouts, 12 ha sitework quantities, labor counts, equipment groups, material allowances, durations and weather are game presets. Purchase packages are paid at order; labor and rentals accrue daily even when idle, plus $12,000/day site overhead. Model time is 12 seconds per scenario day at 1x; 3x and 8x remain available. Permits, strength development, qualification and staffing are substantially simplified. The three-day curing wait is a game allowance, not a strength prediction. Hired workers are rendered individually (up to 225). Equipment and commuter vehicles are representative; the lot is not a parking-capacity constraint in the scheduler.

Campus source files are `web/jobsite-projects.html`, `jobsite-campus.js` (pure scheduler, catalogs, sources and save validation), `jobsite-campus-scene.js` (Three.js), `jobsite-campus-actors.js` (equipment, terrain, workers and parking), `jobsite-campus-activity.mjs` (testable activity/path geometry), `jobsite-campus-ui.js` (controls and persistence), and `jobsite-campus.css` under the corresponding assets folders. Campus rendering is capped at 30 fps and 1.25 device pixels per CSS pixel. The globe uses a local 4096 × 2048 NASA Blue Marble overview and streams sharper NASA GIBS tiles as you zoom. Detail adapts to projected size and screen density, with at most 40 selected patches, four concurrent requests and 48 cached tile textures. Obsolete requests are aborted and failed requests back off. The overview remains visible while detail loads or when the service is unavailable; Natural Earth outlines are the final fallback. Globe rendering uses up to two device pixels per CSS pixel. Narrow screens preserve the globe’s horizontal field of view. Markers on the far side of Earth are hidden, and nearby markers separate for selection. A project list remains available if globe rendering fails. Globe controls are in `jobsite-world-scene.js`, camera/marker math in `jobsite-world-math.mjs`, bounded tile loading in `jobsite-world-imagery.js`, and tested geographic tiling/level selection in `jobsite-world-tiles.mjs`. No new runtime dependencies were added.

Run `npm test` for the simulation, save, globe and construction-activity regressions, including complete automatic runs at all seven destinations. Open `/__activity-check` on the QA server for 21 rendered-scene checks covering arrivals, worker capacity, excavation, haul cycles, trench/backfill, concrete, lifting, pause, reconstruction and all three campus types. With the QA server running, open `/__campus-check` for 31 browser checks: an automatic full build without management or inspection clicks, pause, the read-only roster, saved-incident recovery, map/save return, destination isolation, viewport filling, wheel/keyboard/touch navigation and graphics fallback. Add `?mobile` for three additional portrait/landscape layout checks. Touch checks use synthetic pointer input and should be supplemented with real-device testing. `/__campus-check?preview&world&mobile` previews the portrait world view. `/__campus-check?preview&site=starbase&seed=complete` previews completed civil works; use `site=terafab` for the fab. Fixtures use memory storage and an accelerated local-only clock. `/__campus-migration-check` runs four browser checks for exact original-save backup, retained progress, reload and storage failure. `/__world-detail-check` runs seven live-rendering checks for imagery resolution, close zoom, cross-hemisphere panning, cache limits, return to overview, unavailable imagery and request cancellation. Add `?mobile` for a phone-sized globe.

## Dispatch an autonomous utility spread

Open **Utility sandbox** from the world toolbar (or `jobsite-utility.html` when running locally). Choose a region and equipment spread, then select **Build a utility line**. Click **Start excavation**, **Start pipe crew**, and **Start backfill** to dispatch independent assignments. **Start whole spread** starts all three with one click. No held key is needed.
...
- `web/index.html` and `web/jobsite-projects.html`: full-window world, campus game and learning guide.
- `web/jobsite-utility.html`: utility sandbox and excavator game.
...
Code is [MIT licensed](LICENSE). Three.js and OrbitControls retain their [MIT license](web/assets/vendor/three/LICENSE). NASA Blue Marble Earth imagery retains its NASA credit and usage terms. Natural Earth fallback map data is public domain. Generated landscape and ground textures are included; their source notes and prompts are in [ASSETS.md](web/assets/jobsite/ASSETS.md).

Activity previews on the QA server: `/__campus-fixture?seed=earth`, `seed=concrete`, `seed=steel`, `seed=mid`, `seed=arrival` and `seed=complete`. Choose **Resume project** to inspect the saved phase. Fixtures use memory storage and do not alter a real saved project. `/__activity-check` has a phase selector and run/pause controls for inspecting individual construction cycles.
