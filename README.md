...
The site camera fills the window automatically. **Operations** opens a scrolling drawer for work packages, labor, equipment, procurement and project records; close it to return to the unobstructed site. The superintendent’s desk provides the next dispatch or inspection action. The 3D campus gains foundations, structural bays, enclosure, plant equipment and fit-out as work advances. Use **Roof off** to inspect internal services and racks. Terafab replaces data-hall fit-out with process utilities, cleanrooms and tool installation. Starbase has a pad, tower and integration building; it does not simulate rocket operations. Regional ground and repeating weather presets change outdoor productivity and crane availability.
...
Campus source files are `web/jobsite-projects.html`, `jobsite-campus.js` (pure scheduler, catalogs, sources and save validation), `jobsite-campus-scene.js` (Three.js), `jobsite-campus-ui.js` (controls and persistence), and `jobsite-campus.css` under the corresponding assets folders. Campus rendering is capped at 30 fps and 1.25 device pixels per CSS pixel. The globe uses NASA Blue Marble imagery, renders at up to 1.5 device pixels per CSS pixel, and falls back to Natural Earth outlines if the texture is unavailable. Narrow screens preserve the globe’s horizontal field of view. Markers on the far side of Earth are hidden, and nearby markers separate for selection. A project list remains available if globe rendering fails. Globe controls are in `jobsite-world-scene.js` and the tested camera/marker math is in `jobsite-world-math.mjs`. No new runtime dependencies were added.

Run `npm test` for 114 simulation, save and globe-math checks. With the QA server running, open `/__campus-check` for 23 browser checks: a full build, inspection holds, pause, resource additions, map/save return, destination isolation, viewport filling, wheel/keyboard/touch navigation, operations controls and graphics fallback. Add `?mobile` for three additional portrait/landscape layout checks. Touch checks use synthetic pointer input and should be supplemented with real-device testing. `/__campus-check?preview&world&mobile` previews the portrait world view. `/__campus-check?preview&site=starbase&seed=complete` previews completed civil works; use `site=terafab` for the fab. Fixtures use memory storage and an accelerated local-only clock.
