...
Campus source files are `web/jobsite-projects.html`, `jobsite-campus.js` (pure scheduler, catalogs, sources and save validation), `jobsite-campus-scene.js` (Three.js), `jobsite-campus-actors.js` (equipment, terrain, workers and parking), `jobsite-campus-activity.mjs` (testable activity/path geometry), `jobsite-campus-ui.js` (controls and persistence), and `jobsite-campus.css` under the corresponding assets folders. Campus rendering is capped at 30 fps and 1.25 device pixels per CSS pixel. The globe uses NASA Blue Marble imagery, renders at up to 1.5 device pixels per CSS pixel, and falls back to Natural Earth outlines if the texture is unavailable. Narrow screens preserve the globe’s horizontal field of view. Markers on the far side of Earth are hidden, and nearby markers separate for selection. A project list remains available if globe rendering fails. Globe controls are in `jobsite-world-scene.js` and the tested camera/marker math is in `jobsite-world-math.mjs`. No new runtime dependencies were added.

Run `npm test` for 138 simulation, save, globe and construction-activity checks. Open `/__activity-check` on the QA server for 21 rendered-scene checks covering arrivals, worker capacity, excavation, haul cycles, trench/backfill, concrete, lifting, pause, reconstruction and all three campus types. With the QA server running, open `/__campus-check` for 26 browser checks: a full build, inspection holds, pause, resource additions, map/save return, destination isolation, viewport filling, wheel/keyboard/touch navigation, operations controls and graphics fallback. Add `?mobile` for three additional portrait/landscape layout checks. Touch checks use synthetic pointer input and should be supplemented with real-device testing. `/__campus-check?preview&world&mobile` previews the portrait world view. `/__campus-check?preview&site=starbase&seed=complete` previews completed civil works; use `site=terafab` for the fab. Fixtures use memory storage and an accelerated local-only clock. `/__campus-migration-check` runs four browser checks for exact original-save backup, retained progress, reload and storage failure.
