...
The automatic campaign keeps traffic separation enabled. Earlier saved contacts preserve installed work, the **$25,000 game allowance and ten-point score penalty**, and their half-day stand-down review. Run project starts or resumes that review; work releases automatically when its time has elapsed. Contact is nongraphic. Review restores guarded traffic and regroups personnel. Counts, review state and the latest incident survive save/reload. The low-level simulator still exposes unprotected contact and explicit recovery for regression checks. These numeric penalties, envelopes and compressed recovery stages are gameplay choices, not real costs, safe distances or incident-response procedures. There is no injury/medical model or rigid-body simulation. Parked equipment, decorative plant and overhead loads do not yet have a full physical collision solver.
...
Destination-specific geometry and its factual limits are recorded in [JOBSITE_SITE_MODELS.md](JOBSITE_SITE_MODELS.md). Fresh attempts use researched or explicitly conceptual layouts; earlier saves keep their original structures until the user applies the new model. The same work packages drive the footprint-adjusted excavation, crews, deliveries and installed pieces.


## Automatic superintendent

Start construction runs the campaign through handover without management or acceptance clicks. Operations is a read-only reference for work packages, labor/equipment and deliveries. Pause, speed, cameras and project records remain available. Reading Operations leaves the project running; the field guide, World, leaving the tab and reload pause it.

`startProject`, `supervise` and `advanceProject` wrap the lower-level simulation. They enable unfinished work, place missing material orders once, schedule resources for ready or approaching work, and accept a completed inspection only after its dependencies are accepted. Inspection crews, reach checks, curing, material lead times, weather and traffic holds still consume their normal scenario time. The engine's lower-level dispatch/inspection APIs remain available for tests, without exposing them as required player controls.

Staffing looks 0.75 scenario days ahead through the remaining dependency graph. It counts distinct work areas, caps capacity at the number of rendered work fronts, retains surplus units for at most one day of short-term standby, and removes finished trades. The earthmoving and utility spreads each run one production assignment at a time; up to two concrete/erection/service crews and three access spreads support parallel building fronts. Additions retain the two-day mobilization allowance. Per-unit last-demand timestamps survive saves, so reload does not restart standby or charge another mobilization. This is a scheduling heuristic, not a contractor staffing estimate.

Existing quantities, orders, costs and site layouts are preserved. Previously held work, zero-capacity rosters and completed inspection holds recover when the player resumes. New attempts begin with no hired resources; the automatic superintendent brings in the first crew on Start. All resources demobilize at handover. Node regressions exercise all seven complete campaigns, elapsed inspections, old process saves, pause, costs, reload and incident recovery. The browser campaign finishes without opening Operations or clicking inspection acceptance; rendered interaction checks run all seven destinations with automatic staffing and protected traffic.
